...
def do_measurement():
    ...
    ads1 = ADS1256(pi=io.pi(PI_HOST), conf=card1)
    ads2 = ADS1256(pi=io.pi(PI_HOST), conf=card2)
    ...
    while True:
        for chan, ads in enumerate((ads1, ads2)):
            ### STEP 3: Get data:
            raw_channels = ads.read_sequence(CH_SEQUENCE)
            voltages     = [i * ads.v_per_digit for i in raw_channels]

            ### STEP 4: DONE. Have fun!
            nice_output(chan, raw_channels, voltages)
...
def nice_output(channel, digits, volts):
    sys.stdout.write(
          "\0337" # Store cursor position
        +
f"""
These are the raw sample values for channel {channel}:
...
These are the sample values converted to voltage in V for channel {channel}:
Poti_CH0,  LDR_CH1,     AIN2,     AIN3,     AIN4,     AIN7, Poti NEG, Short 0V
"""
